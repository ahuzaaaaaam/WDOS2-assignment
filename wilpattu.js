...
function getData() {
   const storedData = localStorage.getItem("DataWil");
   if(storedData) {
      ...
      .then(res => res.json())
      .then(data => {
         localStorage.setItem("DataWil", JSON.stringify(data));
         processData(data);
         })
...
function saveChanges() {
   ...
   localStorage.setItem("DataWil", JSON.stringify(cont));
...
function def() {
   const storedData = localStorage.removeItem("DataWil");
   if(storedData) {
       processData(JSON.parse(storedData));
   } else {
       fetch("wilpattu.json")
