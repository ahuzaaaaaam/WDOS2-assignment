...
const yalatitle = document.getElementById("yalatitle")
const willtitle = document.getElementById("willtitle")
...
let edited6sub1 = document.getElementById("edited6sub1")
...
function getData() {
   const storedData = localStorage.getItem("DataAni");
   if(storedData) {
      ...
      .then(res => res.json())
      .then(data => {
         localStorage.setItem("DataAni", JSON.stringify(data));
         processData(data);
         })
...
function processData(res){
   ...
   yalatitle.innerHTML = cont[5].yalatitle
   willtitle.innerHTML = cont[5].willtitle
}
...
function openEditor() {
   ...
   edited5sub1.value = cont[5].yalatitle
   edited6sub1.value = cont[5].willtitle
...
function saveChanges() {
   ...
   cont[5].yalatitle = edited5sub1.value;
   cont[5].willtitle = edited6sub1.value;

   localStorage.setItem("DataAni", JSON.stringify(cont));
...
function def() {
   const storedData = localStorage.removeItem("DataAni");
   if(storedData) {
       processData(JSON.parse(storedData));
   } else {
       fetch("animal.json")
